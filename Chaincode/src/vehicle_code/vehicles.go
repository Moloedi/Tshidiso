...
type Vehicle struct {
	...
	VehicleCustomCert	string	`json:"vehiclecustomcert"`
}
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "create_vehicle" {
		...
		} else if function == "update_vcert"  { return t.update_vcert(stub, v, caller, caller_affiliation, args[0])
...
func (t *SimpleChaincode) create_vehicle(stub shim.ChaincodeStubInterface, caller string, caller_affiliation string, v5cID string) ([]byte, error) {
	...
	vCerti		   := "\"VehicleCustomCert\":\"UNDEFINED\", "
	leaseContract  := "\"LeaseContractID\":\"UNDEFINED\", "
	status         := "\"Status\":0, "
	scrapped       := "\"Scrapped\":false"

	vehicle_json := "{"+v5c_ID+vin+make+model+reg+owner+colour+vCerti+leaseContract+status+scrapped+"}" 	// Concatenates the variables to create the total JSON object
...
//	 update_vehicleCert
//=================================================================================================================================
func (t *SimpleChaincode) update_vcert(stub shim.ChaincodeStubInterface, v Vehicle, caller string, caller_affiliation string, new_value string) ([]byte, error) {
	...
	if 		v.Status			== STATE_MANUFACTURE	&&
			...
			v.Scrapped			== false				{
					...
					v.VehicleCustomCert = new_value
	} else {

        return nil, errors.New(fmt.Sprint("Permission denied. update_vehicleCert %t %t %t" + v.Owner == caller, caller_affiliation == MANUFACTURER, v.Scrapped))
	...
	_, err := t.save_changes(stub, v)
															...
															if err != nil { fmt.Printf("UPDATE_VEHICLE_CERT: Error saving changes: %s", err); return nil, errors.New("Error saving changes") }
