		...
		success: function(d) {
			...
			for(var i = d.transactions.length-1; i >= 0 ; i--)
			{
				...
				if(obj.payload.indexOf("create_vehicle_log") == -1)
				{
					...
					if(payload.indexOf("update_vehicleCert") != -1)
					{
						type = "Update";
						function_name = "update_vehicleCert";
						update_type = "VehicleCustomCert";
					}
					...
					if(type == "Transfer")
					{
						...
						var vcert = get_update("vehicleCert", v5cID);
						var carDetails = '['+vin+'] '+make+' '+model+', '+reg+', '+colour+', '+vcert
