...
function showEditTbl(el)
{
	...
	var vcert = $(el).siblings('.carCert').html()
	if(vcert == '&lt;<i>customCert</i>&gt;')
	{
		vcert = 'undefined'
	}
	$('#vin').val(vin);
	...
	$('#hidVehiclecustomcert').val(vcert);
...
function validate(el)
{
	...
	*/
	...
	if($('#vcert').val().trim().toLowerCase() == 'undefined' && $('hidVehiclecustomcert').val().trim().toLowerCase() != 'undefined')
	{
	...
	{
		$('#errorRw').hide();
		updateAsset($('#vin').val().trim(), $('#make').val().trim(), $('#model').val().trim(), $('#colour').val().trim(), $('#reg').val().trim().toUpperCase(),$('#vcert').val().trim(), $('#v5cID').val(), el)
	}
	else
	{
		$('#errorRw').show();
